...
class TestComponentInitialization:
    ...
    def test_get_components_returns_all_five(self):
        """Test get_components returns index, vault, redis, graphiti, livegrep."""
        # Reset global state
    ...
    def test_get_components_without_graphiti(self):
        """Test get_components works when Graphiti is not available."""
        import memory_mcp.server_sdk as sdk_module
        sdk_module._index = None
        sdk_module._vault = None
        sdk_module._redis = None
                ...
                with patch('memory_mcp.server_sdk.REDIS_AVAILABLE', False):
                    with patch('memory_mcp.server_sdk.GRAPHITI_AVAILABLE', False):
                        with patch('memory_mcp.server_sdk.LIVEGREP_AVAILABLE', False):
                            ...
                            # Graphiti should be None since GRAPHITI_AVAILABLE is False
                            assert graphiti is None
                            assert livegrep is None

    def test_get_components_with_redis_only(self):
        """Test get_components with only Redis available."""
        import memory_mcp.server_sdk as sdk_module
        sdk_module._index = None
        sdk_module._vault = None
        sdk_module._redis = None
                    ...
                    with patch('memory_mcp.server_sdk.RedisClient'):
                        with patch('memory_mcp.server_sdk.GRAPHITI_AVAILABLE', False):
                            with patch('memory_mcp.server_sdk.LIVEGREP_AVAILABLE', False):
                                ...
                                assert vault is not None
                                assert redis is not None
                                # Graphiti and livegrep are optional
                                assert graphiti is None
                                assert livegrep is None
        ...
        with patch('memory_mcp.server_sdk.SQLiteIndex'):
            with patch('memory_mcp.server_sdk.VaultManager'):
                with patch('memory_mcp.server_sdk.REDIS_AVAILABLE', False):
                    with patch('memory_mcp.server_sdk.GRAPHITI_AVAILABLE', False):
                        with patch('memory_mcp.server_sdk.LIVEGREP_AVAILABLE', False):
                            index1, vault1, _, _, _ = get_components()
                            index2, vault2, _, _, _ = get_components()
...
class TestMemoryStoreHandler:
    ...
    async def test_store_with_all_fields(self):
        ...
        with patch('memory_mcp.server_sdk.get_components', return_value=(mock_index, mock_vault, None, None, None)):
...
class TestSessionSaveHandler:
    ...
    async def test_save_with_redis(self):
        ...
        with patch('memory_mcp.server_sdk.get_components', return_value=(mock_index, mock_vault, mock_redis, None, None)):
...
class TestMemoryStatsHandler:
    ...
    async def test_stats_all_components(self):
        ...
        # Test without graphiti (since it requires async mocking)
        with patch('memory_mcp.server_sdk.get_components', return_value=(mock_index, mock_vault, mock_redis, None, None)):
    ...
    async def test_stats_minimal_components(self):
        ...
        with patch('memory_mcp.server_sdk.get_components', return_value=(mock_index, mock_vault, None, None, None)):
            ...
            assert len(result) == 1
            assert "Memory Statistics" in result[0].text
            assert "N/A" in result[0].text  # Redis and Graphiti
